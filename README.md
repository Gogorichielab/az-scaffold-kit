...
Releases are semver git tags. See [CHANGELOG.md](CHANGELOG.md) for what changed, and [AGENTS.md](AGENTS.md#versioning-and-releases) for what counts as a major, minor, or patch bump.

Tags cover the whole repository, docs included, so a doc-only release still bumps the tag. What `?ref=` guarantees is that the module's behaviour is fixed, not that every tag changed it.

## Also in this repo

- [AGENTS.md](AGENTS.md) — conventions for AI assistants: Terraform practices, semantic versioning rules, verification steps, and known constraints.
- [docs/free-azure-services.md](docs/free-azure-services.md) — free-tier cheat sheet of what each Azure service gives you at no cost, carrying a **Last verified** date and a revision history. Check that date before trusting a figure, and follow [Keeping this current](docs/free-azure-services.md#keeping-this-current) when re-verifying.
- [CHANGELOG.md](CHANGELOG.md) — notable changes to this project.
